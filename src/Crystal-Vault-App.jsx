...
const CHECKOUT_URL = "https://checkout-the-crystal.vercel.app"; // update to your real checkout URL
...
const inp        = { width:"100%", background:B.ice, border:`1px solid ${B.border}`, borderRadius:8, padding:"11px 14px", fontSize:13, color:B.text, fontFamily:F, boxSizing:"border-box", outline:"none" };
const card       = { background:B.white, border:`1px solid ${B.border}`, borderRadius:12, boxShadow:B.shadow };
...
function AuthScreen({ onAuth }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:B.ice, display:"flex", alignItems:"center", justifyContent:"center", padding:24, fontFamily:F }}>
      <div style={{ maxWidth:420, width:"100%", ...card, padding:"40px 36px", textAlign:"center" }}>
        ...
        {sent ? (
          <>
            <p style={{ fontSize:13, color:B.textMuted, lineHeight:1.75, marginBottom:20, fontFamily:F }}>Check your inbox. We sent a magic link to <strong>{email}</strong>. Click it to access your vault.</p>
            <button onClick={() => setSent(false)} style={{ ...ghostBtn, fontSize:12 }}>Use a different email</button>
          </>
        ) : (
          <>
            <p style={{ fontSize:13, color:B.textMuted, lineHeight:1.75, marginBottom:24, fontFamily:F }}>Enter the email you used when purchasing your Fragments. We'll send you a magic link — no password needed.</p>
            <input value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleSend()} placeholder="your@email.com" type="email" autoFocus style={{ ...inp, marginBottom:12, textAlign:"center", fontSize:14 }}/>
            {err && <div style={{ fontSize:12, color:"#E24B4A", marginBottom:10, fontFamily:F }}>{err}</div>}
            <button onClick={handleSend} disabled={loading||!email.trim()} style={{ ...primaryBtn, padding:"12px 0", width:"100%", fontSize:14, opacity:loading||!email.trim()?0.5:1 }}>
              {loading ? "Sending..." : "Send Magic Link →"}
            </button>
            <div style={{ marginTop:16, fontSize:11, color:B.textLight, fontFamily:F }}>Don't have an account yet?{" "}
              <a href={CHECKOUT_URL} style={{ color:B.lightBlue, textDecoration:"none", fontWeight:700 }}>Acquire Fragments →</a>
            </div>
          </>
        )}
      </div>
...
// ── Agent modal (chat interface) ──────────────────────────────────────────────
function AgentModal({ agent, user, onClose }) {
  ...
  // Load agent config (system prompt from Supabase = .md file content)
  // Load existing conversation history
  useEffect(() => {
    const init = async () => {
      // Fetch agent's system prompt from crystal_agent_configs
      ...
      // Fetch existing conversation from crystal_agent_sessions
      const { data: session } = await supabase
        .from("crystal_agent_sessions")
        .select("messages")
        .eq("user_id", user.id)
        .eq("agent_id", agent.id)
        .maybeSingle();
  ...
  const saveSession = async (updatedMessages) => {
    // Upsert the conversation — one row per user+agent pair
    await supabase.from("crystal_agent_sessions").upsert(
      { user_id: user.id, agent_id: agent.id, agent_name: agent.name, messages: updatedMessages, updated_at: new Date().toISOString() },
  ...
  const send = async () => {
    ...
    try {
      // Call Claude API with this agent's system prompt (from Supabase / .md file)
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1024,
          system: systemPrompt,   // ← this IS the .md file content, loaded from Supabase
      ...
      await saveSession(finalMessages);   // persist conversation to Supabase
    } catch (e) {
    ...
    <div style={{ position:"fixed", inset:0, zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      ...
      <div style={{ position:"relative", background:B.white, border:`1px solid ${B.border}`, borderRadius:16, width:"100%", maxWidth:620, height:"80vh", display:"flex", flexDirection:"column", boxShadow:"0 24px 60px rgba(10,22,40,0.18)", overflow:"hidden", fontFamily:F }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"20px 20px 10px" }}>
          {loadingHistory ? (
            <div style={{ textAlign:"center", padding:40, color:B.textLight, fontSize:12, fontFamily:F }}>Loading your conversation...</div>
          ) : messages.length === 0 ? (
            <div style={{ textAlign:"center", padding:"32px 20px" }}>
              <div style={{ fontSize:32, marginBottom:12 }}>{agent.icon}</div>
              <div style={{ fontSize:15, fontWeight:700, color:B.text, fontFamily:F, marginBottom:6 }}>Start talking to {agent.name}</div>
              <div style={{ fontSize:12, color:B.textMuted, lineHeight:1.7, fontFamily:F, maxWidth:320, margin:"0 auto" }}>{agent.desc}</div>
              <div style={{ marginTop:16, fontSize:11, fontStyle:"italic", color:B.textLight, fontFamily:F }}>{agent.tagline}</div>
            </div>
          ) : (
            messages.map((m, i) => (
              <div key={i} style={{ display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start", marginBottom:16 }}>
                {m.role === "assistant" && (
                  <div style={{ width:28, height:28, borderRadius:7, background:B.skyBlue, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, color:B.lightBlue, flexShrink:0, marginRight:8, marginTop:2 }}>{agent.icon}</div>
                )}
                <div style={{ maxWidth:"78%", padding:"10px 14px", borderRadius:m.role==="user"?"12px 12px 4px 12px":"12px 12px 12px 4px", background:m.role==="user"?`linear-gradient(135deg,${B.navy},${B.midBlue})`:B.ice, color:m.role==="user"?"#fff":B.text, fontSize:13, lineHeight:1.7, fontFamily:F, whiteSpace:"pre-wrap" }}>
                  {m.content}
                </div>
              </div>
            ))
        ...
        <div style={{ padding:"12px 16px", borderTop:`1px solid ${B.border}`, flexShrink:0 }}>
          {!systemPrompt && !loadingHistory && (
            <div style={{ fontSize:11, color:"#E24B4A", textAlign:"center", marginBottom:8, fontFamily:F }}>Agent config not loaded — check Supabase crystal_agent_configs table</div>
          )}
          <div style={{ display:"flex", gap:8, alignItems:"flex-end" }}>
            <textarea
              ...
              rows={1}
              style={{ flex:1, background:B.ice, border:`1px solid ${B.border}`, borderRadius:8, padding:"10px 12px", fontSize:13, color:B.text, fontFamily:F, outline:"none", resize:"none", maxHeight:120, lineHeight:1.5 }}
            />
          ...
          <div style={{ fontSize:9, color:B.textLight, fontFamily:F, marginTop:6, textAlign:"center", letterSpacing:"0.5px" }}>Enter to send · Shift+Enter for new line</div>
        </div>
      </div>
...
// ── Petal card (unlocked = hover reveal + activate, locked = greyed out) ───────
function PetalCard({ agent, unlocked, onActivate }) {
  ...
  return (
    <div
      ...
      onClick={() => unlocked && onActivate(agent)}
      style={{
        ...card,
        padding:"20px",
        cursor: unlocked ? "pointer" : "default",
        opacity: unlocked ? 1 : 0.5,
        border: `1px solid ${hov ? B.lightBlue : B.border}`,
        boxShadow: hov ? B.shadowHover : B.shadow,
        transform: hov ? "translateY(-4px)" : "none",
        transition: "all 0.22s",
        position: "relative",
        overflow: "hidden",
        background: hov ? B.skyBlue : B.white,
      }}
    >
      {/* Hover shimmer */}
      {hov && (
        <div style={{ position:"absolute", inset:0, background:`linear-gradient(135deg,${B.lightBlue}06,${B.skyBlue}40)`, borderRadius:12, pointerEvents:"none" }}/>
      )}

      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10, position:"relative" }}>
        <div style={{ width:36, height:36, borderRadius:8, background:hov?`${B.lightBlue}22`:B.skyBlue, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, color:hov?B.lightBlue:B.textLight, transition:"all 0.2s" }}>
          {agent.icon}
        </div>
      ...
      {unlocked ? (
        <>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:6, color:B.lightBlue }}>
            <span style={{ fontSize:12, fontWeight:700, fontFamily:F }}>{hov ? "Open Agent →" : "Activate"}</span>
          </div>
        </>
      ) : (
...
// ── User profile / vault view ──────────────────────────────────────────────────
function VaultView({ user, fragments, onLogout }) {
  const [activeAgent, setActiveAgent] = useState(null);

  // Which agent IDs does this user have unlocked?
  ...
  return (
    <div style={{ maxWidth:1100, margin:"0 auto", padding:"40px 28px 80px", fontFamily:F }}>
      ...
      <div style={{ background:`linear-gradient(135deg,${B.navy},${B.midBlue})`, borderRadius:14, padding:"22px 28px", marginBottom:28, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:14 }}>
        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:18, fontWeight:700, color:"#fff", fontFamily:F }}>{user.email}</div>
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          ...
          <button onClick={onLogout} style={{ ...ghostBtn, color:"rgba(255,255,255,0.5)", border:"1px solid rgba(255,255,255,0.15)" }}>Exit Vault</button>
        </div>
      </div>

      {/* Stats */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:12, marginBottom:28 }}>
        {[["Agents Unlocked", unlockedIds.length, true],["Agents Locked", 12-unlockedIds.length, false],["Fragments", fragments.length, false],["Status", unlockedIds.length>0?"Active":"Inactive", true]].map(([l,v,ac]) => (
      ...
      <div style={{ fontSize:10, fontWeight:700, letterSpacing:"1.5px", textTransform:"uppercase", color:B.textLight, marginBottom:16, fontFamily:F }}>
        Your 12 Petals — {unlockedIds.length} Unlocked
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))", gap:12 }}>
        {AGENTS.map(a => (
          <PetalCard key={a.id} agent={a} unlocked={unlockedIds.includes(a.id)} onActivate={setActiveAgent}/>
        ))}
      </div>

      {/* Upsell if not all unlocked */}
      {unlockedIds.length < 12 && (
        <div style={{ ...card, marginTop:24, padding:"18px 22px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:14 }}>
          <div>
            <div style={{ fontSize:14, fontWeight:700, color:B.text, marginBottom:3, fontFamily:F }}>{12-unlockedIds.length} agents still locked</div>
            <div style={{ fontSize:12, color:B.textMuted, fontFamily:F }}>Acquire more Fragments to unlock the full Crystal.</div>
          </div>
          <a href={CHECKOUT_URL} target="_blank" rel="noopener noreferrer" style={{ ...primaryBtn, textDecoration:"none" }}>Acquire More Fragments →</a>
        </div>
      )}

      {/* Agent modal */}
      {activeAgent && <AgentModal agent={activeAgent} user={user} onClose={() => setActiveAgent(null)}/>}
    </div>
  );
}
  ...
  return (
    <div style={{ maxWidth:1100, margin:"0 auto", padding:"40px 28px 80px", fontFamily:F }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:14, marginBottom:40 }}>
        {[["12","AI Agents"],["4","Fragment Tiers"],["∞","Potential"],["1×","Payment"]].map(([v,l]) => (
      ...
      {/* Architecture */}
      <div style={{ marginBottom:40 }}>
        <div style={{ fontSize:10, fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase", color:B.lightBlue, marginBottom:6, fontFamily:F }}>The Architecture</div>
        <h2 style={{ fontSize:22, fontWeight:700, color:B.text, marginBottom:20, fontFamily:F }}>Fragment → Petal → Agent</h2>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))", gap:14 }}>
          {[
            { icon:"◆", title:"The Fragment", desc:"Your access key. Purchase 1, 3, 6, or 12. Choose which agents to unlock. One unique code per agent delivered to your email." },
            { icon:"⬡", title:"The Petal",    desc:"Each petal is a fully-operational AI agent. Specialised. Precise. Built for one specific function inside your media operation." },
            { icon:"∞", title:"The Crystal",  desc:"All 12 petals working as one unified intelligence. The complete Prëmo AI stack." },
          ].map(item => (
            <div key={item.title} style={{ ...card, padding:"22px" }}>
              <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
                ...
                <div style={{ fontSize:14, fontWeight:700, color:B.text, fontFamily:F }}>{item.title}</div>
              </div>
              <div style={{ fontSize:12, color:B.textMuted, lineHeight:1.7, fontFamily:F }}>{item.desc}</div>
            </div>
          ))}
        </div>
      </div>

      {/* Agent grid — all locked on landing */}
...
export default function App() {
  ...
  const [view, setView]           = useState("landing"); // landing | auth | vault
  const [loading, setLoading]     = useState(true);

  // Check for existing Supabase session on load
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) { setUser(session.user); loadFragments(session.user); setView("vault"); }
      else setLoading(false);
    });
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      if (session?.user) { setUser(session.user); loadFragments(session.user); setView("vault"); }
      else { setUser(null); setFragments([]); setView("landing"); }
    });
    return () => subscription.unsubscribe();
  }, []);

  const loadFragments = async (u) => {
    // Find the user's crystal_users row by email
    const { data: crystalUser } = await supabase
      .from("crystal_users")
      .select("id")
      .eq("email", u.email)
      .maybeSingle();
  ...
  return (
    <div style={{ fontFamily:F, minHeight:"100vh", background:B.ice, color:B.text }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500;600;700;800;900&display=swap'); *{box-sizing:border-box;margin:0;padding:0;} body{background:${B.ice};font-family:'Montserrat',sans-serif;} button:hover:not(:disabled){opacity:0.85;} a:hover{opacity:0.85;} input::placeholder{color:${B.textLight};} textarea::placeholder{color:${B.textLight};} textarea:focus,input:focus{outline:none;} ::-webkit-scrollbar{width:4px;} ::-webkit-scrollbar-track{background:transparent;} ::-webkit-scrollbar-thumb{background:${B.border};border-radius:2px;}`}</style>
      ...
      <header style={{ background:B.white, borderBottom:`1px solid ${B.border}`, padding:"12px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, cursor:"pointer" }} onClick={() => setView(user?"vault":"landing")}>
          <div style={{ width:36, height:36, borderRadius:9, background:B.navy, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
            <CrystalMark size={28}/>
          </div>
          <div>
      ...
      {/* Views */}
      {loading && <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"60vh", color:B.textLight, fontSize:12, letterSpacing:"2px", fontFamily:F }}>Loading...</div>}
      {!loading && view === "landing" && <Landing onEnter={() => setView("auth")}/>}
      {!loading && view === "auth"    && <AuthScreen onAuth={(u) => { setUser(u); setView("vault"); }}/>}
      {!loading && view === "vault"   && user && <VaultView user={user} fragments={fragments} onLogout={logout}/>}

      {view !== "vault" && (
        <div style={{ textAlign:"center", padding:"24px", fontSize:10, color:B.textLight, fontFamily:F, letterSpacing:"1px", borderTop:`1px solid ${B.border}`, background:B.white }}>
