...
const CHECKOUT_URL = "https://checkout-the-crystal.vercel.app"; // update after deploy
...
const TIERS = [
  { count:1,  price:97,   label:"1 Fragment",   badge:null,           desc:"Your first agent. Activate the one that matters most right now.", cta:"Get 1 Fragment" },
  { count:3,  price:297,  label:"3 Fragments",  badge:null,           desc:"Three agents. The creative core begins.", cta:"Get 3 Fragments" },
  { count:6,  price:597,  label:"6 Fragments",  badge:"Most Popular", desc:"Half the vault. Production, strategy, and scale.", cta:"Get 6 Fragments" },
  { count:12, price:1297, label:"12 Fragments", badge:"Full Crystal", desc:"All twelve agents. The full Rosa ecosystem. One mind.", cta:"Get the Fragments" },
];
  ...
  return (
    <div style={{ minHeight:"100vh", background:B.ice, display:"flex", alignItems:"center", justifyContent:"center", padding:24, fontFamily:F }}>
      <div style={{ maxWidth:420, width:"100%", ...card, padding:"40px 36px", textAlign:"center" }}>
        ...
        {sent ? (
          <>
            <p style={{ fontSize:13, color:B.textMuted, lineHeight:1.75, marginBottom:20, fontFamily:F }}>Check your inbox. We sent a magic link to <strong>{email}</strong>.</p>
          ...
          <>
            ...
            <div style={{ marginTop:16, fontSize:11, color:B.textLight, fontFamily:F }}>
...
function AgentModal({ agent, user, onClose }) {
  ...
  useEffect(() => {
    const init = async () => {
      const { data: config } = await supabase.from("crystal_agent_configs").select("system_prompt").eq("agent_id", agent.id).single();
      if (config) setSystemPrompt(config.system_prompt);
      const { data: session } = await supabase.from("crystal_agent_sessions").select("messages").eq("user_id", user.id).eq("agent_id", agent.id).maybeSingle();
  ...
  const saveSession = async (msgs) => {
    await supabase.from("crystal_agent_sessions").upsert(
      { user_id:user.id, agent_id:agent.id, agent_name:agent.name, messages:msgs, updated_at:new Date().toISOString() },
    ...
    <div style={{ position:"fixed", inset:0, zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      ...
      <div style={{ position:"relative", background:B.white, border:`1px solid ${B.border}`, borderRadius:16, width:"100%", maxWidth:620, height:"80vh", display:"flex", flexDirection:"column", boxShadow:"0 24px 60px rgba(10,22,40,0.18)", overflow:"hidden", fontFamily:F }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"20px 20px 10px" }}>
          {loadingHistory ? (
            <div style={{ textAlign:"center", padding:40, color:B.textLight, fontSize:12, fontFamily:F }}>Loading conversation...</div>
          ) : messages.length === 0 ? (
            <div style={{ textAlign:"center", padding:"32px 20px" }}>
              ...
              <div style={{ marginTop:14, fontSize:11, fontStyle:"italic", color:B.textLight, fontFamily:F }}>{agent.tagline}</div>
            </div>
          ) : messages.map((m,i) => (
        ...
        <div style={{ padding:"12px 16px", borderTop:`1px solid ${B.border}`, flexShrink:0 }}>
          {!systemPrompt && !loadingHistory && <div style={{ fontSize:11, color:"#E24B4A", textAlign:"center", marginBottom:8, fontFamily:F }}>Agent config not found — check crystal_agent_configs in Supabase</div>}
          <div style={{ display:"flex", gap:8, alignItems:"flex-end" }}>
            <textarea ref={textareaRef} value={input} onChange={e=>{ setInput(e.target.value); e.target.style.height="auto"; e.target.style.height=Math.min(e.target.scrollHeight,120)+"px"; }} onKeyDown={onKey} placeholder={`Message ${agent.name}...`} rows={1} style={{ flex:1, background:B.ice, border:`1px solid ${B.border}`, borderRadius:8, padding:"10px 12px", fontSize:13, color:B.text, fontFamily:F, outline:"none", resize:"none", maxHeight:120, lineHeight:1.5 }}/>
...
function PetalCard({ agent, unlocked, onActivate }) {
  ...
  return (
    <div
      ...
      onMouseLeave={() => unlocked && setHov(false)}
      onClick={() => unlocked && onActivate(agent)}
      style={{ ...card, padding:"20px", cursor:unlocked?"pointer":"default", opacity:unlocked?1:0.5, border:`1px solid ${hov?B.lightBlue:B.border}`, boxShadow:hov?B.shadowHover:B.shadow, transform:hov?"translateY(-4px)":"none", transition:"all 0.22s", position:"relative", overflow:"hidden", background:hov?B.skyBlue:B.white }}
    >
...
function TierCard({ tier }) {
  ...
  return (
    <div
      ...
      style={{ ...card, padding:"22px", cursor:"pointer", border:`1px solid ${hov?B.lightBlue:B.border}`, boxShadow:hov?B.shadowHover:B.shadow, transform:hov?"translateY(-3px)":"none", transition:"all 0.2s", position:"relative" }}
      onClick={() => window.open(`${CHECKOUT_URL}?count=${tier.count}`, "_blank")}
    >
      {tier.badge && (
        <div style={{ position:"absolute", top:-10, left:"50%", transform:"translateX(-50%)", fontSize:9, fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase", padding:"3px 12px", borderRadius:20, background:hov?B.navy:B.lightBlue, color:"#fff", fontFamily:F, whiteSpace:"nowrap" }}>
          {tier.badge}
        </div>
      )}
      {/* Petal dots */}
      <div style={{ display:"flex", gap:3, flexWrap:"wrap", marginBottom:14 }}>
        {AGENTS.map(a => (
          <div key={a.id} style={{ width:9, height:9, borderRadius:2, background:a.id<=tier.count?B.lightBlue:B.skyBlue }}/>
        ))}
      </div>
      <div style={{ fontSize:9, fontWeight:600, color:B.textLight, fontFamily:F, marginBottom:4 }}>{tier.count} of 12 petals</div>
      <div style={{ fontSize:26, fontWeight:900, color:B.text, letterSpacing:"-1px", fontFamily:F, marginBottom:4 }}>${tier.price.toLocaleString()}</div>
      <div style={{ fontSize:12, color:B.textMuted, fontFamily:F, lineHeight:1.6, marginBottom:16 }}>{tier.desc}</div>
      <div style={{ fontSize:10, fontWeight:700, color:hov?B.green:B.textLight, fontFamily:F }}>One-time · Lifetime access</div>
      <button style={{ ...primaryBtn, width:"100%", padding:"10px 0", marginTop:14, fontSize:12 }}>
        {tier.cta}
      </button>
    </div>
  );
    ...
    <div style={{ maxWidth:1100, margin:"0 auto", padding:"40px 28px 80px", fontFamily:F }}>
      ...
      {unlockedIds.length < 12 && (
        <div style={{ marginTop:28 }}>
          <div style={{ fontSize:10, fontWeight:700, letterSpacing:"1.5px", textTransform:"uppercase", color:B.textLight, marginBottom:6, fontFamily:F }}>Acquire More Fragments</div>
          <div style={{ fontSize:10, color:B.textMuted, fontFamily:F, marginBottom:20 }}>{12-unlockedIds.length} agents still locked. Add to your vault.</div>
...
function Landing({ onEnter }) {
  return (
    <div style={{ maxWidth:1100, margin:"0 auto", padding:"40px 28px 80px", fontFamily:F }}>
      ...
      {/* ── Fragment tiers — "Pick your depth" ── */}
      <div>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:20, flexWrap:"wrap", gap:10 }}>
          <div>
          ...
          <p style={{ fontSize:12, color:B.textMuted, fontFamily:F }}>One-time purchase. Lifetime access. No subscriptions.</p>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(210px,1fr))", gap:14 }}>
          {TIERS.map(t => <TierCard key={t.count} tier={t}/>)}
        </div>
...
export default function App() {
  ...
  const loadFragments = async (u) => {
    const { data: crystalUser } = await supabase.from("crystal_users").select("id").eq("email", u.email).maybeSingle();
    if (crystalUser) {
      const { data: frags } = await supabase.from("crystal_fragments").select("*").eq("user_id", crystalUser.id).eq("active", true).order("purchased_at", { ascending:true });
      setFragments(frags || []);
    }
    setLoading(false);
  };
